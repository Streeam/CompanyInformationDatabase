...
public class UserResource {
    ...
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<UserDTO> updateUserAndCreateEmployee(@Valid @RequestBody UserDTO userDTO) {
        log.debug("REST request to update User and create employee : {}", userDTO);
        Optional<User> existingUser = userRepository.findOneByEmailIgnoreCase(userDTO.getEmail());
        if (existingUser.isPresent() && (!existingUser.get().getId().equals(userDTO.getId()))) {
            throw new EmailAlreadyUsedException();
        }
        ...
        Optional<UserDTO> updatedUser = userService.updateUserAndCreateEmployee(userDTO);
