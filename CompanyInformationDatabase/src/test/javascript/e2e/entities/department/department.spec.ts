...
describe('Department e2e test', () => {
  ...
  /*  it('should create and save Departments', async () => {
        async function createDepartment() {
            await departmentComponentsPage.clickOnCreateButton();
            await departmentUpdatePage.setDepartmentInput('department');
            expect(await departmentUpdatePage.getDepartmentInput()).to.match(/department/);
            await departmentUpdatePage.siteSelectLastOption();
  ...
  /*  it('should delete last Department', async () => {
        ...
        expect(await departmentDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/cidApp.department.delete.question/);
