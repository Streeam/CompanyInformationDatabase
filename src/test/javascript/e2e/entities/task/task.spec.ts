...
describe('Task e2e test', () => {
  ...
  it('should create and save Tasks', async () => {
    async function createTask() {
      await taskComponentsPage.clickOnCreateButton();
      await taskUpdatePage.setTaskDescriptionInput('taskDescription');
      expect(await taskUpdatePage.getTaskDescriptionInput()).to.match(/taskDescription/);
      await taskUpdatePage.setStartDateInput('01-01-2001');
      expect(await taskUpdatePage.getStartDateInput()).to.eq('2001-01-01');
      await taskUpdatePage.setEndDateInput('01-01-2001');
      expect(await taskUpdatePage.getEndDateInput()).to.eq('2001-01-01');
      await taskUpdatePage.setProgressInput('5');
      expect(await taskUpdatePage.getProgressInput()).to.eq('5');
      await taskUpdatePage.statusSelectLastOption();
      await taskUpdatePage.prioritySelectLastOption();
      await taskUpdatePage.setNonconformanceIdInput('5');
      expect(await taskUpdatePage.getNonconformanceIdInput()).to.eq('5');
      await taskUpdatePage.setEmployeeIdInput('5');
      expect(await taskUpdatePage.getEmployeeIdInput()).to.eq('5');
      await taskUpdatePage.amendmentSelectLastOption();
  ...
  it('should delete last Task', async () => {
    ...
    expect(await taskDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/cidApp.task.delete.question/);
