...
export const ACTION_TYPES = {
  ...
  CUSTOMER_BATCH: 'customer/CUSTOMER_BATCH'
};
...
export default (state: CustomerState = initialState, action): CustomerState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.CUSTOMER_BATCH):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.CUSTOMER_BATCH):
      return {
        ...state,
        loading: false,
        updating: false,
    ...
    case SUCCESS(ACTION_TYPES.CUSTOMER_BATCH):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const createEntities = (listOfListCustomers: ICustomer[]) => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.CUSTOMER_BATCH,
    payload: axios.post(apiUrl + `/batches`, listOfListCustomers)
  });
  return result;
};
